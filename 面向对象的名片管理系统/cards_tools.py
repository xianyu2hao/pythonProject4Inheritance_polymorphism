...
class Cardstool:
    ...
    def show_all(self):
        ...
        for item in self.card_list:  # 临时变量获取的是列表中的字典
    ...
    def search_card(self):
        ...
        for item in self.card_list:  # item是字典
            if item.get('name') == find_name:
                ...
                self.deal_card(item)  # 将字典传到了deal_card的函数
                break
        else:
            print("没有找到姓名为：%s 的名片，请核对后重新输入!!！" % find_name)
    # 5.对调用的函数进行选择操作
    def deal_card(self,find_dict):  # 将item字典传到find_dict中
        """
        选择1是修改名片
        选择2是删除名片
        选择0是返回上一级
        ...
        if op == "1":
            ...
            change_name = self.input_card(find_dict.get('name'),"请输入修改后的姓名[不修改直接回车]:")
            change_phone = self.input_card(find_dict.get('phone'),"请输入修改后的手机[不修改直接回车]:")
            change_qq = self.input_card(find_dict.get('qq'),"请输入修改后的qq号[不修改直接回车]:")
            change_email = self.input_card(find_dict.get('email'),"请输入修改后的邮箱[不修改直接回车]:")
            # 此处可以将change的几个临时变量直接删除，让find.dict['name']=input_card..get('name'),"请输入修改后的姓名[不修改直接回车]:")
            ...
            print("输入有误，请核对后重新输入！！！")
    #6.
    def input_card(self,find_dict,msg): # 对input函数进行扩充
        info = input(msg)
        if len(info) > 0:
            print(info)
            return (info)
    ...
    def save_card_data(self):
        ...
        # 将输入的名片列表的数据输入到文件中
        card_file_txt.write(str(self.card_list)) # 不能将列表直接写入文件，需要转换为字符串
        card_file_txt.close()
    # 确保下次启动可以导入之前保存的数据
    ...
    def load_files_data(self):
        ...
        if  os .path.exists("card_data.txt"):
          ...
          # 操作文件
            ...
            # 将列表修改为全局变量
            # global card_list
            self.card_list = eval(card_files1)  # 读取的是字符串，不能直接装进列表中，需要eval()函数转换；另外eval函数执行空会错误，故需要先判断
            # 关闭文件
            card_file.close()
        else:
            print("保存名片列表不存在，请新建！")